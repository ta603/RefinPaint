...
class REMIPlusInpainting(MIDITokenizer):
    ...
    *Pitch*, *Velocity* and *Duration* tokens, and time with *Bar* and *Position* tokens.
    ...
    def __notes_to_events(self, tracks: List[Instrument], midi_miner) -> List[Event]:
        ...
        if self.additional_tokens.get("Chord", False):  # "Chord" in additional tokens
            ...
            maximum_time = max(n[0].end for n in notes_with_program) + ticks_per_beat * 4
            chords = []
            for chord_point in range(0, int(maximum_time + 1), ticks_per_beat * 2):
                ch = Event(time=chord_point, value="None", desc="None", type="Chord")
                chords.append(ch)
    ...
    def _midi_to_tokens(self, midi: MidiFile, path="", *args, **kwargs) -> TokSequence:
        ...
        """
        assert "midi_miner" in kwargs, "Missing json_chords"
        # Convert each track to tokens
        events = self.__notes_to_events(midi.instruments, kwargs["midi_miner"])
    ...
    def tokens_to_midi(
        ...
        for ti, token in enumerate(tokens):
            if token.split("_")[0] == "Bar":
                ...
                try:
                    if (
                        tokens[ti - 3].split("_")[0] == "Position"
                        ...
                        vel = 65 if tokens[ti - 2].split("_")[1] != "melody" else 75
            ...
            elif "Chord_None" in token:
                if pcps is None:
                    pcp_type = "N"
                else:
                    pcp_type = self._str_pcp(pcps[number_chord])
                markers.append(
                    Marker(text=pcp_type, time=current_tick)
                )
                number_chord += 1
        if len(tempo_changes) > 1:
