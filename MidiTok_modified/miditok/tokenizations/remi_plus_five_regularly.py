...
import numpy as np
from miditoolkit import Instrument, MidiFile, Note, TempoChange, TimeSignature, Marker, Lyric

from MidiTok_modified.miditok.tokenizations.normalization_chords import normalization_chord
...
class REMIPlusFiveRegularly(MIDITokenizer):
    ...
    *Pitch*, *Velocity* and *Duration* tokens, and time with *Bar* and *Position* tokens.
    ...
    def __init__(
        ...
        additional_tokens["Function"] = True
    ...
    def _quantize_timestamp(self, time: float) -> float:
        r"""Quantize the time.

        :param time: time to quantize in seconds.
        """
        time_division = self._current_midi_metadata["time_division"]
        tempo_changes = [(t.tempo, t.time) for t in self._current_midi_metadata['tempo_changes']]

        # only one tempo in the song
        if len(tempo_changes) == 1:
            ticks = time * tempo_changes[0][0] / 60 * time_division
            return ticks

        # time changes
        tempo_changes_seconds, previous_tempo_time, previous_timestamp = [], 0, 0
        for ii, (tempo, tempo_time) in enumerate(tempo_changes):
            new_time_stamp = (tempo_time - previous_tempo_time) / (tempo / 60 * time_division)
            tempo_changes_seconds.append(previous_timestamp + new_time_stamp)
            previous_tempo_time = tempo_time
            previous_timestamp = previous_timestamp + new_time_stamp

        ticks = 0
        for ii, ((tempo, _), tempo_time_seconds) in enumerate(zip(tempo_changes, tempo_changes_seconds)):
            # miramos el siguiente y si se pasa de segundos, nos quedamos con el presente si no acumulamos
            if ii + 1 == len(tempo_changes) or time < tempo_changes_seconds[ii + 1]:
                ticks += (time - tempo_time_seconds) * tempo / 60 * time_division
                break
            else:
                ticks += (tempo_changes_seconds[ii + 1] - tempo_time_seconds) * tempo / 60 * time_division

        return ticks

    def _detect_chords_plus_plus(self, chord_txt, chord_map, unkknown_chord, chord_maps):
        chords = []
        # read tsv file chord_txt
        with open(chord_txt, "r") as f:
            for line in f:
                line = line.rstrip("\n")
                line = line.split("\t")
                normalized_chord = normalization_chord(line[2])
                chords.append(
                    Event(
                        type="Chord",
                        value=normalized_chord,
                        time=self._quantize_timestamp(float(line[0])),
                        desc=chord_maps[normalized_chord.split(':')[-1]] if normalized_chord != 'None' else 'N',
                    )
                )
        return chords

    def __notes_to_events(self, tracks: List[Instrument], chord_file, midi_miner) -> List[Event]:
        ...
        """
        program2function = {v: k for k, v in midi_miner.items()}
        # Flatten all notes
        notes_with_program = [
        ...
        if self.additional_tokens.get("Chord", False):  # "Chord" in additional tokens
            chords_detected = self._detect_chords_plus_plus(
                chord_file,
                self.additional_tokens["chord_maps"],
                self.additional_tokens["chord_unknown"],
                chord_maps=self.additional_tokens["chord_maps"]
            )
            ticks_per_beat = self._current_midi_metadata["time_division"]
            chords = []

            # only chords
            for chord_point in range(0, int(chords_detected[-1].time + 1), ticks_per_beat * 2):
                c = max([c for c in chords_detected if c.time <= chord_point], key=lambda x: x.time)
                ch = Event(time=chord_point, value=c.value, desc=c.desc, type=c.type)
                chords.append(ch)
            # chords.extend(chords_detected)
            ...
            for chord in chords:
                ...
                events.append(
                    Event(
                        "Function",
                        "drum" if tracks[0].is_drum else program2function[tracks[0].program],
                        chord.time,
                        "FunctionProgramChord"
                    )
                )
                events.append(
                    Event("Program", tracks[0].program, chord.time, "ProgramChord")
                )
        ...
        for note, (program_num, is_drum) in notes_with_program:
            ...
            events.append(
                Event(
                    type="Function",
                    value="drum" if is_drum else program2function[program_num],
                    time=note.start,
                    desc=note.pitch,
                )
            )
    ...
    def track_to_tokens(self, track: Instrument) -> TokSequence:
        r"""TODO WHAT IS DOING THIS FUNCTION?
        Converts a track (miditoolkit.Instrument object) into a sequence of tokens (:class:`miditok.TokSequence`).
    ...
    def _midi_to_tokens(self, midi: MidiFile, path="", *args, **kwargs) -> TokSequence:
        r"""Converts a preprocessed MIDI object to a sequence of tokens.

        :param midi: the MIDI objet to convert.
        :return: sequences of tokens.
        """
        assert "chords_file" in kwargs and "midi_miner" in kwargs, "Missing json_chords or midi_miner"
        # Convert each track to tokens
        events = self.__notes_to_events(midi.instruments, kwargs["chords_file"], kwargs["midi_miner"])
    ...
    def tokens_to_midi(
        ...
        for ti, token in enumerate(tokens):
            if token.split("_")[0] == "Bar":
                ...
                try:
                    if (
                        ...
                        instruments[id_func].notes.append(
            ...
            elif token.split("_")[0] == "Chord":
                chord_type = str(tokens[ti].split("_")[1])
                markers.append(
                    Marker(text=chord_type, time=current_tick)
                )
        if len(tempo_changes) > 1:
        ...
        chord_instrument = Instrument(
            program=1,
            is_drum=False,
            name="Chords conditioning",
        )
        for marker in markers:
            chord_instrument.notes.append(
                Note(velocity=1, pitch=71, start=marker.time, end=TIME_DIVISION)
            )
        midi.lyrics = markers

        midi.instruments = []
        midi.instruments.append(chord_instrument)
        for function_name, function_number in self.function2id.items():
            if function_number in instruments.keys():
                midi.instruments.append(instruments[function_number])
    ...
    def _create_base_vocabulary(
        ...
        if self.additional_tokens["Function"]:
            vocab += [f"Function_{f}" for f in ['melody', 'bass', 'accompaniment', 'chord', 'drum']]
