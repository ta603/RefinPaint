...
class REMIPlusPlus(MIDITokenizer):
    ...
    *Pitch*, *Velocity* and *Duration* tokens, and time with *Bar* and *Position* tokens.
    ...
    def __notes_to_events(self, tracks: List[Instrument], chord_file, melody_program) -> List[Event]:
        ...
        if self.additional_tokens.get("Chord", False):  # "Chord" in additional tokens
            for track in tracks:  # find chords per track
                if track.program != melody_program:
                    continue
        ...
        # TODO HERE I COULD REMOVE THE PROGRAMS DUPLICATES
        return events
    ...
    def _midi_to_tokens(self, midi: MidiFile, path="", *args, **kwargs) -> TokSequence:
        ...
        """
        assert "chords_file" in kwargs and "melody_program" in kwargs, "Missing json_chords or melody_program"
        # Convert each track to tokens
        events = self.__notes_to_events(midi.instruments, kwargs["chords_file"], kwargs["melody_program"])
    ...
    def tokens_to_midi(
        ...
        for ti, token in enumerate(tokens):
            if token.split("_")[0] == "Bar":
                ...
                program = int(tokens[ti - 2].split("_")[1])
