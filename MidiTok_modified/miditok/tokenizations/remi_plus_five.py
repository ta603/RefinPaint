...
class REMIPlusFive(MIDITokenizer):
    ...
    def __notes_to_events(self, tracks: List[Instrument], chord_file, midi_miner) -> List[Event]:
        ...
        if self.additional_tokens.get("Chord", False):  # "Chord" in additional tokens
            for track in tracks:  # find chords per track
                if track.program != midi_miner["melody"]:
                    continue

                chords = self._detect_chords_plus_plus(
                    chord_file,
                ...
                for chord in chords:
                    ...
                    events.append(
                        Event(
                            "Function",
                            "drum" if track.is_drum else program2function[track.program],
                            chord.time,
            ...
            if token.split("_")[0] == "Bar":
                ...
                try:
                    if (
                        ...
                        and tokens[ti - 1].split("_")[0] == "Function"
                    ):
                ...
                markers.append(
                    Lyric(text=chord_type, time=current_tick)
                )
        if len(tempo_changes) > 1:
